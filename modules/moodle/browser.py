...
class Browser:
    ...
    async def get_cookies_moodle(self, user_id, barcode, passwd) -> tuple[dict, bool, str, str]:
        try:
            ...
            self.wait.until(voel((By.XPATH, '//*[@id="i0116"]'))).send_keys(barcode + "@astanait.edu.kz")
            self.wait.until(voel((By.XPATH, '//*[@id="idSIButton9"]'))).click()

            try:
                self.wait.until(voel((By.XPATH, '//*[@id="i0118"]'))).send_keys(passwd)
            except Exception:
                username_error = self.driver.find_element(By.XPATH, '//*[@id="usernameError"]')
            ...
            try:
                self.wait.until(
                    voel((By.XPATH, '//*[@id="lightbox"]/div[3]/div/div[2]/div/div[3]/div[1]/div/label/span'))
                )
            except Exception:
                try:
                    password_error = self.driver.find_element(By.XPATH, '//*[@id="passwordError"]')
                    ...
                    self.wait.until(voel((By.XPATH, '//*[@id="CancelLinkButton"]'))).click()
            self.wait.until(voel((By.XPATH, '//*[@id="idSIButton9"]'))).click()

            self.wait.until(voel((By.XPATH, '//*[@id="action-menu-toggle-1"]/span/span[2]/span/img')))
    ...
    def get_token_du(self):
        ...
        self.driver.get(url)
        self.wait.until(voel((By.XPATH, '//*[@id="root"]/section/section/header/div/div[3]/div/div/a/span[1]')))
